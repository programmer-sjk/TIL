...
- 코드를 읽을 일이 많아진다는 것은 **`가독성을 높여야 생산성을 개선한다는 의미이다`**.
...
- `onMessageReceived` 이름은 **`호출하는 쪽에서 무엇을 하는지 알 수가 없어서 세부 구현까지 읽어야 한다`**.
  - 호출자 입장에서 언제 호출하는지 이미 알고 있으므로 이름의 정보가 도움이 되질 않는다.
...
- 실제 인수와 반환 값의 상세한 예를 보여주는 문서화 주석
...
- **`두 변수가 서로에게 영향을 주지 않는다면`**, 이 변수의 관계를 **직교**라고 한다.
- 반대로 직교하지 않는 관계. 서로에게 영향을 준다면 비직교로 정의한다.
- 직교 관계를 추구하는 것은 가독성과 견고함을 높이기 위한 중요한 기법이다.
...
- 이름이 없는 리터럴을 **`매직넘버라고`** 한다.
  ...
  - 배열 인덱스의 맨 앞을 나타내는 `0`이나, 다음을 구하는 `+1`, 중간을 나타내는 `/ 2`는 모두 의미가 명확하다.
...
- Calculator는 `Caller`가 다음 제약 사항을 충족할 것을 기대한다.
...
- 위 코드에서 userList는 `UserListPresenter`에게 중요한 정보지만, `UserListPresenter` 입장에선 어떤 클래스가 `userList`를 관리하는지 알 수 없다.
- 위 문제를 해결하기 위해 `userList` 변경에 대한 책임을 `UserListPresenter`에게만 부여하자.
...
- 위 예제에서 userList는 `UserListPresenter`를 통해서만 변경이 가능하고 **`외부에서 변경하는 것을 허용하지 않는다`**.
...
- B에 생성자로 A 인스턴스가 전달되지만, A 인스턴스의 초기화가 완료되지 않았다면 `a.i`를 출력하는 코드에서 버그가 발생한다.
...
- `Caller` 클래스가 `Callee` 클래스를 호출하면 `Caller`가 `Callee`에게 의존한다고 볼 수 있다.
- 만약 `Callee`도 `Caller`를 호출해 의존성이 순환하게 되면, 해결하기 위한 **`첫 번째 방법은 작은 클래스로 나누는 것이다`**.
...
- `ArrayIntList`의 `addElement` 동작이 부모인 `IntList`에 구현되어 있다.
  - 따라서 `ArrayIntList` 구현에 변경이 필요하면 `IntList`도 변경해야 한다.
  - 자식 클래스의 구현이 늘어나면 `IntList.addElement`도 커지게 된다.
...
- 의존 관계도 중복될 수 있다. A,B가 클래스 C에 대해 공통적으로 의존하는 경우 중복될 수 있다.
- 이는 피할수 없는 일이지만, 불필요한 연결된 의존 관계에 대해 알아보자.
- 의존성의 방향이 아래와 같다고 가정해보자.
  - `ProfileImagePresenter -> NicknamePresenter -> UserModelProvider`
  - 여기서 `ProfileImagePresenter`가 `NicknamePresenter`에 의존하고 있지만, 단지 `UserModelProvider`를 얻기 위해서다.
- **`단지 참조를 얻기 위한 목적으로 관련 없는 클래스에 의존하면`** 영향 범위를 파악하기 어려워진다.
- 이런 경우 연결된 의존성이 아닌, **`생성자나 인자를 통해 명시적으로 의존성을 전달할 수 있다`**.

#### 디미터의 법칙

- 직접 의존성 사용을 좀 더 정리한 개념으로 디미터의 법칙이 있다.
- 디미터의 법칙은 `여러 개의 .(도트)을 사용하지 말라는` 법칙으로도 알려져 있고, 이 법칙을 통해 의존 관계를 제거할 수 있다.

## 코드 리뷰

- 효율적인 리뷰를 위해, 리뷰를 요청하는 리뷰이와 리뷰를 하는 리뷰어 모두 주의해야 할 것이 있다.

### 리뷰하기 쉬운 Pull Request 만들기

- 리뷰하기 쉬운 `Pull Request`를 만들기 위해 아래 세 가지가 중요하다.
  - 목적이 명확하다.
  - 크기가 작다.
  - 포함된 커밋이 구조적이다.
- 만약 **`큰 기능을 개발하게 되면 하향식으로 분할하는 방법을 써볼 수 있다`**.
  - 하향식 방식에서는 **`세부적인 부분은 구현하지 않고 클래스의 스켈레톤 코드만 작성한다`**.
  - 스켈레톤 코드에서는 다른 클래스와의 의존 관계와 대략적인 인터페이스, TODO 코멘트로 예상되는 구현을 기술한다.
  - 스켈레톤 코드로 전체적인 구조와 설계를 먼저 리뷰할 수 있어서, 리뷰의 정밀도를 높이는데 도움이 된다.

### 리뷰 커멘트 적용하기

- 리뷰에서 **`제안을 무조건 수용하기 보다, 토론할 수 있는 환경을`** 함께 만들어 나가는게 이상적이다.
- 리뷰이가 커멘트를 코드에 적용할 때 주의해야 할 세 가지를 살펴보자.
  - 잘못된 의견이나 질문이 나온 원인 찾기
    - 리뷰어가 잘못 이해했거나 의문을 가졌다는 건, 다른 개발자가 코드를 읽을 때도 동일한 오해나 의문을 가질 수 있다.
    - 리뷰어가 오해를 했다면, 왜 오해를 했는지 살펴보고 확인해야 한다.
  - 제안의 의도 파악하기
    - 제안된 코드를 수용하기에 앞서, 그 코드를 적용하면 무엇이 개선되는지 파악해야 한다.
  - 제안을 다른 부분에 적용할 수 있을지 살펴보기

### 리뷰어의 원칙

- 언어 폭력이나 비난이 섞인 지적은 리뷰가 아니며, 리뷰는 리뷰이를 돕는 과정이란 것을 명심해야 한다.
- **`리뷰의 목적은 코드베이스의 가독성을 높이고 일정 수준의 생산 효율을 유지하는데 있다`**.
  - 따라서 리뷰에 지나치게 많은 시간을 할애하면 목적에 상반된다.
- 리뷰어가 신경써야 할 네 가지를 살펴보자.
  - 요청받은 리뷰를 방치하지 않기
  - 문제 있는 리뷰 거부하기
  - 기한을 지나치게 의식하지 않기
  - 제안이 아닌 의견을 제시하기
...
- 리뷰를 요청 받으면 최대 언제까지 (24시간 이내) 답변할지 팀의 규칙을 정해두는 것이 좋다.
- 리뷰어는 우선순위에 따라 리뷰를 바로 할 순 없어도, 언제까지 할 수 있다는 상황을 전달할 필요가 있다.

#### 기한을 지나치게 의식하지 않기

- 바빠서, **`마감일이 임박해서의 이유로 리뷰 품질을 낮추면 안 된다`**.
- 예외가 반복되어 저 품질의 리뷰가 지속된다면, 제품 품질이 심각한 수준까지 떨어질 수 있다.

#### 제안이 아닌 의견 제시하기

- **`코드의 개선을 제안할 때는 구체적이고 명확한 내용으로 전달해야 한다`**.
- 제안을 모호하게 작성하면 리뷰이는 해당 커멘트를 어떻게 처리해야 할지 몰라 부담을 느낄 수 있다.
- 제안 내용이 명확하면 리뷰이가 취해야 할 행동도 분명해지므로, 리뷰와 코드 수정이 되풀이되는 과정을 줄일 수 있다.
