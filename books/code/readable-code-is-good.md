...
- 경계를 포함하는 한계갑을 다룰 때는 min/max를 사용하자.
- 경계를 포함하는 범위에는 first/last를 사용하자.
- 경계를 포함하고 배제하는 범위에는 begin/end를 사용하자.
...
- 값의 상한과 하한을 정할때는 min/max가 좋은 접두사 역할을 한다.
- 경계를 포함한다면 first/last를, 경계의 마지막을 배제한다면 begin/end가 널리 사용되는 이름이다.
