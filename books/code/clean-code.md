...
- **`중복 제거는 이 책에 나오는 가장 중요한 규칙 중 하나이다`**. 코드에서 중복을 발견할 때 마다 추상화할 기회로 간주해라. 중복된 코드를 하위 루틴이나 다른 클래스로 분리하라. 추상화로 중복을 정리하면 설계 언어의 어휘가 늘어난다. 가장 뻔한 유형은 똑같은 코드가 중복되어 보이는 현상이다. 이런 중복은 간단한 함수로 교체한다. 좀 더 미묘한 유형은 switch/case나 if/else 문으로 같은 조건을 거듭 확인하는 중복이다. 이런 중복은 다형성으로 대체해야 한다. 최근 나온 디자인 패턴은 대다수가중복을 제거하는 잘 알려진 방법에 불과하다.
