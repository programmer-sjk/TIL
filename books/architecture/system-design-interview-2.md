...
- 시스템이 비동기적으로 통신하는 경우 메시지가 전달되거나 응답이 반환된다는 보장이 없다. 시스템 성능을 높이기 위해 비동기 통신을 자주 사용하는 결제 사업에 일반적인 문제다. PSP나 은행도 비동기 통신을 선호한다. 그렇다면 **`어떻게 정확성을 보장할 수 있을까?`**
...
- **`내부 서비스 통신에는 동기와 비동기 방식이 있다`**.
- HTTP와 같은 동기식 통신은 소규모 시스템에서는 잘 작동하지만 규모가 커지면 단점은 분명해진다. 동기식 통신에서 한 요청에 응답을 만드는 처리 주기는 관련된 서비스가 많아질수록 길어진다. 또한 성능 저하, 높은 결합도, 낮은 확장성의 문제도 있다.
- 비동기 통신은 메시지 큐를 통해 비동기적으로 처리하는 방법이다. 카프카를 사용할 경우 여러 서비스가 받아서 처리할 수 있다. 만약 하나의 요청이 푸시 알림, 재무 보고 업데이트, 분석 결과 업데이트 등 다양한 용도에 쓰인다면 카프카가 적합하다.
- 일반적으로 동기시 통신은 설계하기 쉽지만, 서비스의 자율성을 높이기에는 적당하지 않다. 비지니스 로직이 복잡하고 타사 서비스 의존성이 높은 대규모 결제 시스템에는 비동기 통신이 더 나은 선택이다.

#### 결제 실패 처리

- 모든 결제 시스템은 실패한 결제를 적절히 처리할 수 있어야 한다.
- 결제 상태 추적
  - 결제 주기의 모든 단계에서 결제 상태를 정확하게 유지하는 것은 매우 중요하다. 실패가 일어날 때마다 결제 거래의 현재 상태를 파악하고 재시도 또는 환불이 필요한지 여부를 결정한다. 결제 상태는 DB 테이블에 보관한다.
- 재시도 큐 및 실패 메시지 큐
  - 실패를 우아하게 처리하기 위해서는 재시도 큐와 실패 메시지 큐를 두는 것도 바람직하다.
  - 재시도 큐는 일시적 오류 같은 재시도 가능한 오류를 보낸다.
  - 실패 메시지 큐는 반복적으로 처리에 실패한 경우 보낸다. 이 큐는 문제가 있는 메시지를 디버깅하고 격리하여 실패한 이유를 파악하기 위한 검사에 유용하다.

#### 정확히 한 번 전달

- **`결제 시스템에 발생할 수 있는 심각한 문제로 고객에게 이중으로 청구하는 문제가 있다`**.
- 결제 주문이 정확히 한 번만 실행되도록 결제 시스템을 설계하는 것이 중요하다.
- 재시도
  - 간혹 네트워크 오류나 시간 초과로 결제 거래를 다시 시도해야 하는 경우가 있다.
  - 재시도 메커니즘을 활용하면 결제가 최소 한 번은 실행되도록 보장 가능하다.
  - 가령 클라이언트가 결제를 시도하지만 네트워크 문제로 결제가 자꾸 실패하는 경우 재시도를 하는 방안이다.
  - 재시도시 발생할 수 있는 문제는 이중 결제로 아래의 경우가 있다.
    - 결제 시스템이 외부 결제 페이지를 통해 PSP와 연동하는 환경에서 결제 버튼이 두 번 중복 클릭된다.
    - PSP가 결제를 성공적으로 하였으나 네트워크 오류로 응답이 결제 시스템에 도달하지 못했다. 사용자가 결제 버튼을 다시 클릭한다.
  - **`이중 결제를 방지하려면 결제는 멱등성을 가져야 한다`**.
- 멱등성
  - **`멱등성은 여러번 실행해도 결과가 그대로 보존되는 특성을 일컫는다`**. API 관점에서는 API 호출을 여러번 해도 동일한 결과가 나온다는 뜻이다.
  - 사용자가 결제를 클릭하면 멱등키가 HTTP 요청의 일부로 결제 시스템에 전송된다. 결제 시스템이 두 번째 요청을 받으면 요청에 포함된 멱등키를 이전에 받았다는 것을 알기에 처리하지 않거나 현재 상태를 반환한다.
  - 멱등성을 지원하는 한 가지 방법은 DB 제약 조건을 활용하는 것이다. 예를 들어 DB의 기본 키를 멱등키로 사용한다.
