...
- 할 일을 백로그로 관리하든 업무 도구를 사용하던 기능 개발로 몽땅 채우지는 말자. **`새로운 기능 외에도 해야 할 일들은 아래와 같다`**.
  - 구형 시스템 유지 보수
  - 프로세스 회고와 개선
  - 새로운 기술 탐험
  - 학습 및 기술 갈고 닦기
...
- 스포티파이, 넷플릭스, 깃랩 등과 같이 성공한 회사의 정책과 프로세스를 앞다투어 도입하고 있다. 각자 나름의 방식으로 SW를 개발하고 관리한다. **`하지만 여러분의 회사가 동일한 시장과 동일한 제약 조건을 갖고 있고, 비슷한 전문성과 조직 크기, 비슷한 경영진과 문화를 보유하고 있는가?`** 속아 넘어가지 말자. 피상적인 구조나 정책, 프로세스, 방법론 만으로는 부족하다. 근본적인 실용주의 기법을 하나 택해서 적용해 보자. 작은 팀이나 조직 하나에서 아이디어를 시험해 보자. 잘 맞는 것 같으면 좋은 부분만 유지하고 아니면 버리면 된다. 이것이 성공한 회사들의 비결이다.
- 진짜 목표는 스크럼을 하거나 애자일을 한다가 아니다. **`진짜 목표는 동작하는 SW를 제공함으로써 사용자가 즉각적으로 새로운 일을 할 수 있게 되는 것이다`**. 몇 달, 몇 년 후가 아니라 바로 지금. 지속적 배포가 이상적이지만 도달 불가능한 목표라고 생각하는 팀이 있다. 특히 배포를 몇 달 몇 주에 한 번하는 프로세스를 따르고 있다면 더욱 그러 것이다. 하지만 모든 목표가 그렇듯 계속 올바른 방향을 바라보는 것이 중요하다.

### 실용주의 시작 도구

- 모든 팀에게 필요한 **`가장 기본적이고 중요한 요소 세 가지를 다룬다`**. 버전 관리, 회귀 테스트, 전체 자동화
- **`빌드와 배포 설정도 버전 관리 시스템 안에 있으므로 실제 서비스에 릴리즈하는 것도 자동으로 처리된다`**. 이것이 중요하다. 프로젝트 차원에서는 버전 관리 시스템이 빌드와 릴리즈 프로세스를 운용한다. 버전 관리 시스템에 푸시로 빌드와 테스트, 배포가 시작된다. 빌드는 클라우드 컨테이너에서 돌아간다. 릴리즈 절차는 간단해져서 일상의 일부가 된다. 빌드 장비나 개발자의 장비에 의존하지 않는 진정한 배포가 가능해진다.
- **`일찍 테스트하고, 자주 테스트하고, 자동으로 테스트하자`**. 훌륭한 프로젝트에는 제품 코드보다 테스트 코드가 더 많을 수도 있다. 테스트 코드를 만들기 위해 들이는 시간에는 가치가 있다. 길게 보면 테스트 코드르 위한 시간이 싸게 먹히며 결함이 거의 없는 제품을 만드는 꿈이 이뤄지기도 한다. 빌드 과정에서 단위, 통합, 유효성, 성능 테스트 등이 들어가야 한다. 이것만 갖춘다고 완벽한 것은 아니지만 이 목록은 훌륭한 출발점이 된다.
- 테스트에서 가장 뻔하고 중요한 개념을 말한다. 버그가 기존 테스트의 그물을 빠져나갔다면 다음번에는 그걸 잡아낼 수 있도록 새 테스트를 추가해야 한다. 한번 인간 테스터가 버그를 찾았다면 더는 인간이 그 버그를 만나게 해서는 안 된다.
- **`모든 것을 자동화에 의존하자`**. 빌드 전체가 자동화되어 있지 않다면 클라우드 서버에서 프로젝트를 빌드할 수 없을 것이다. `이거 하나만...` 이라는 생각으로 수작업 단계를 넣는다면 아주 커다란 창문을 깨뜨리는 것이다.
- 버전 관리, 가차 없는 자동 테스트, 전체 자동화라는 세 기둥이 있다면 우리 프로젝트에 필수적인 견고한 기반이 생긴 것이다.

### 사용자를 기쁘게 하라

- **`개발자로서의 우리 목표는 사용자를 기쁘게 하는 것이다`**. 사용자를 기쁘게 하고 싶다면 고객이 문제를 풀 때 적극적으로 도와줄 수 있는 관계를 구축하라. **`소프트웨어 개발자일지 몰라도 진정한 여러분의 직함은 문제 해결사다`**. 이것이 우리가 하는 일이고, 실용주의 프로그래머의 본질이다. 우리는 문제를 해결한다.

### 오만과 편견

- 실용주의 프로그래머는 책임을 회피하지 않는다. 대신 도전을 수용하고 자신의 전문성이 널리 알려지는 것을 기뻐한다. 설계 혹은 코드를 맡는다면 자신이 보기에 자랑스러운 작품을 만들어 낼 것이다. **`옛 장인들은 자신의 작품에 서명하는 것을 자랑스러워 했다. 여러분도 그래야 한다`**.
- 경계심 때문에 우리들의 코드를 참견하는 사람으로부터 방어하려고 해서는 안 된다. 같은 맥락에서 다른 사람들의 코드를 존중해야 한다. 이 팁이 효과를 보려면 개발자 사이의 황금률 (남에게 대접 받고자 하는 대로 너희도 남에게 대접하라)과 상호 존중이라는 기반이 꼭 필요하다.
- 우리는 소유권에 대한 긍지를 보고 싶다. **`내가 이걸 만들었고, 내 작품의 품질을 보증합니다`**. 사람들이 코드에 붙은 여러분의 이름을 보고 그것이 튼튼하고 잘 작성되었으며 제대로 테스트되었을 뿐만 아니라 훌륭히 문서화되었을 것이라고 기대하게 만들자. **`전문가가 만든 진정으로 전문가다운 결과물. 실용주의 프로그래머`**.
