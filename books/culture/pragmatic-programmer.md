...
- 결합된 코드는 바꾸기 힘들다. 코드의 한 곳을 바꾸면 다른 곳에 여파르 미칠수가 있다.
...
- 인터페이스가 강력한 이유는 이들을 타입으로 사용할 수 있고 해당 인터페이스를 구현하는 클래스라면 무엇이든 그 타입과 호환되기 때문이다. 만약 Car와 Phone이 Locatable 인터페이스를 구현했다면 둘 다 Localtable한 것을 담는 리스트에 넣을 수 있다.
